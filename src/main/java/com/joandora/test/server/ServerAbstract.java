...
public abstract class ServerAbstract {
	...
	public static final Log LOG = LogFactory.getLog(ServerAbstract.class);

	private static final ThreadLocal<ServerAbstract> SERVER = new ThreadLocal<ServerAbstract>();
	 ...
	 * the server context.
	 */
	public static ServerAbstract get() {
		return SERVER.get();
	}
    ...
    private void cleanupConnections(boolean force) {
      if (force || numConnections > thresholdIdleConnections) {
		...
		public void run() {
			...
			SERVER.set(ServerAbstract.this);
			while (running) {
				SelectionKey key = null;
				try {
					selector.select();
  ...
  public class Connection {
		...
		public int readAndProcess() throws IOException, InterruptedException {
			while (true) {
				...
				if (data == null) {
					...
					if (dataLength == JDClient.PING_CALL_ID) {//心跳信息
	 ...
	 */
	protected ServerAbstract(String bindAddress, int port, int handlerCount)
			throws IOException {
